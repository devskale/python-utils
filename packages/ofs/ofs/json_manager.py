...
def read_json_file(project: str, filename: str, key_path: Optional[str] = None) -> Union[Dict[str, Any], Any]:
    ...
    if filename not in supported_files:
        raise ValueError(f"Unsupported filename '{filename}'. Supported files: {supported_files}")
    ...
    if filename == 'projekt.json':
        ...
        if not os.path.exists(json_file_path):
            ...
            if bidder_dirs:
                ...
                raise FileNotFoundError(f"No audit.json found in project '{project}'")
    ...
    try:
        ...
        raise json.JSONDecodeError(f"Invalid JSON in {json_file_path}: {str(e)}", e.doc, e.pos)
...
def read_audit_json(project: str, bidder: str, key_path: Optional[str] = None) -> Union[Dict[str, Any], Any]:
    ...
    if not os.path.exists(audit_file_path):
        raise FileNotFoundError(f"audit.json not found for bidder '{bidder}' in project '{project}': {audit_file_path}")
    ...
    try:
        ...
        raise json.JSONDecodeError(f"Invalid JSON in {audit_file_path}: {str(e)}", e.doc, e.pos)
    ...
    for key in keys:
        ...
        if key.isdigit() and isinstance(current, list):
            ...
            if index >= len(current):
                raise KeyError(f"Array index {index} out of range in path '{key_path}'")
            ...
            raise KeyError(f"Cannot access key '{key}' in non-dict/non-list object in path '{key_path}'")
...
def _set_nested_value(data: Dict[str, Any], key_path: str, value: Any) -> None:
    ...
    for key in keys[:-1]:
        ...
        if key.isdigit() and isinstance(current, list):
            index = int(key)
            if index >= len(current):
                raise ValueError(f"Array index {index} out of range in path '{key_path}'")
            current = current[index]
        elif isinstance(current, dict):
            if key not in current:
            ...
            raise ValueError(f"Cannot access key '{key}' in non-dict/non-list object in path '{key_path}'")
    ...
    if final_key.isdigit() and isinstance(current, list):
        ...
        raise ValueError(f"Cannot set key '{final_key}' in non-dict/non-list object in path '{key_path}'")