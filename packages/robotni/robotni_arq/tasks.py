...
def load_subprocess_config() -> Dict:
    ...
    if _subprocess_config is None:
        ...
        try:
            ...
            print(
                f"Warning: Subprocess config file not found at {config_path}")
    ...
    for part in command:
        if '${' in part:
            ...
            try:
                ...
                raise ValueError(
                    f"Missing required parameter {e} for command substitution")
        else:
...
async def another_fake_task(ctx, params: dict):
    ...
    # Handle both old and new parameter structures for backward compatibility
    if 'param' in params:
        # New structure: single param field - use default value
...
async def task_anonymize(ctx, params: dict):
    ...
    # Handle both old and new parameter structures for backward compatibility
    if 'param' in params:
        # New structure: single param field
        param_value = params.get('param', 'NONETEXT')
        # For simplicity, use the param value as text_to_anonymize
        text_to_anonymize = param_value if param_value else 'NONETEXT'
        level = 'basic'  # Default level
    else:
        # Old structure: separate parameters
        text_to_anonymize = params.get('text_to_anonymize', 'NONETEXT')
        level = params.get('level', 'basic')

    logger.info(
        f"Executing task_anonymize with context: {ctx}, text: {text_to_anonymize}, level: {level}")
    ...
    if level == 'basic':
        ...
        anonymized_text = re.sub(
            r'\b\w+\b',
            lambda m: m.group(0)[0] + '*' * (len(m.group(0)) -
                                             1) if len(m.group(0)) > 1 else m.group(0),
            text_to_anonymize
        )
    elif level == 'full':
