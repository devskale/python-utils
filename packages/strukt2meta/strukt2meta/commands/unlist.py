...
class UnlistCommand(BaseCommand):
    ...
    def _process_files(self, items_to_process: list, json_file_path: Path) -> None:
        ...
        for i, item in enumerate(items_to_process, 1):
            path = item.get('path', '')
            prompt_name = self._determine_prompt_for_path(path)
            self.log(f"Processing {i}/{len(items_to_process)}: {path} (prompt: {prompt_name})")
    ...
    def _process_single_item(self, item: dict, json_file_path: Path) -> bool:
        ...
        try:
            ...
            success = self._update_pdf2md_index(result, path, base_directory)
            
            # If we have a target JSON file for injection, also inject there
            if hasattr(self.args, 'target_json') and self.args.target_json:
                target_success = self._inject_metadata(result, path, self.args.target_json)
    ...
    def _determine_prompt_for_path(self, file_path: str) -> str:
        ...
        for part in path_parts:
            if part == 'A':
                if self.verbose:
                    self.log(f"Using 'adok' prompt for /A/ directory: {file_path}")
                return 'adok'
            elif part == 'B':
                if self.verbose:
                    self.log(f"Using 'bdok' prompt for /B/ directory: {file_path}")
                return 'bdok'
        
        # Fallback to the user-specified prompt or default
        fallback_prompt = self.args.prompt
        if self.verbose:
            self.log(f"Using fallback prompt '{fallback_prompt}' for: {file_path}")
    ...
    def _update_pdf2md_index(self, result: dict, file_path: str, base_directory: Path) -> bool:
        """
    ...
    def _print_summary(self, successful: int, failed: int, total: int) -> None:
        ...
        if successful > 0:
            print(f"   📄 Metadata written to .pdf2md_index.json files in each file's directory")
            
            # Also mention target JSON if specified
            if hasattr(self.args, 'target_json') and self.args.target_json:
                print(f"   📄 Additional metadata written to: {self.args.target_json}")