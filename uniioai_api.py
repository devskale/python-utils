...
async def stream_response_generator(messages: List[Dict], provider_model: str, temp: float, max_tok: int) -> AsyncGenerator[str, None]:
    """Generates OpenAI-compatible SSE chunks from uniioai.stream_completion."""
    completion_id = f"chatcmpl-{uuid.uuid4()}"
    created_time = int(time.time())
    model_name = provider_model  # Keep the combined name for the response
    ...
    try:
        async for content_chunk in stream_completion(messages, provider_model, temp, max_tok):
        ...
        # In a real scenario, you might want more specific error handling here
        # For now, we'll just stop the stream and rely on the main endpoint handler
        # to catch the exception if it happens before the first yield.
        # If it happens mid-stream, the client might get an incomplete response.
        print(f"Error during streaming: {e}")
        # Optionally yield an error chunk (though not standard OpenAI)
        # error_chunk = {"error": {"message": str(e), "type": type(e).__name__}}
...
async def chat_completions(request_input: ChatCompletionRequestInput, raw_request: Request):
    ...
    try:
        if request_input.stream:
            ...
            full_content = await get_completion(
                messages=messages_dict,
...
if __name__ == "__main__":
    ...
    # --- Specific Examples for Arli ---

    # Non-streaming (Arli):
    # curl -X POST http://localhost:8000/v1/chat/completions -H "Content-Type: application/json" -d '{"model": "arli@Mistral-Nemo-12B-Instruct-2407", "messages": [{"role": "user", "content": "What is the capital of France?"}], "stream": false, "temperature": 0.5, "max_tokens": 50}'

    # Streaming (Arli):
    # curl -N -X POST http://localhost:8000/v1/chat/completions -H "Content-Type: application/json" -d '{"model": "arli@Mistral-Nemo-12B-Instruct-2407", "messages": [{"role": "system", "content": "You are a helpful assistant."}, {"role": "user", "content": "Write a short poem about the moon."}], "stream": true, "temperature": 0.8}'
